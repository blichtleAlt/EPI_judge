...
void DutchFlagPartition(int pivot_index, vector<Color> &A_ptr) {
  ...
  Color pivot_value = A_ptr[pivot_index];

  int begin = 0, end = A_ptr.size() - 1;

  for(int i = 0; i < A_ptr.size(); ++i){
      if(A_ptr[i] < pivot_value){
        std::swap(A_ptr[i], A_ptr[begin++]);
      }
  } // put elements smaller than pivot at the beginning

  for(int i = A_ptr.size(); i >= 0; --i){
      if(A_ptr[i] > pivot_value){
        std::swap(A_ptr[i], A_ptr[end--]);
      }
  } // put elements greater than pivot at the end
